...
const store = {
  async load() {
    ...
    return r?.value ? migrate(JSON.parse(r.value)) : null;
  },
  async saveAll(data) {
    const r = await window.storage.set(KEY, JSON.stringify(data));
    if (!r) throw new Error("storage unavailable");
    return r;
  },
  async pref(k, v) {
    if (v === undefined) { const r = await window.storage.get(`${KEY}:${k}`); return r?.value ?? null; }
    return window.storage.set(`${KEY}:${k}`, v);
  },
};
